...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, R(rd) = s->snpc; s->dnpc = src1 + imm; /* s->dnpc &= 0xfffffffe; */ if (rd == 1) FTRACE_JUMP(s->dnpc);printf("I jump to "FMT_PADDR" make the addr to %d, in the "FMT_PADDR"\n",s->dnpc,rd,s->pc));
