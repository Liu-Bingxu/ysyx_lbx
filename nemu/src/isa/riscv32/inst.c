...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 == 0) ? 1 : (src1 < imm) ? 1 : 0;);
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm;);
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = src1 + imm; s->dnpc &= 0xfffffffe;);
    ...
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((long long)((sword_t)src1 * (sword_t)src2) >> MUXDEF(CONFIG_ISA64, 64, 32)););
    ...
    INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = ((src1 == 0) ? ((src2 != 0) ? 1 : 0) : ((src1 < src2) ? 1 : 0)););
